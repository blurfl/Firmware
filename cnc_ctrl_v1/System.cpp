      ...
      default :
        ...
        switch ( cmdString[char_counter] ) {
          ...
          case 'R' : // Restore defaults [IDLE/ALARM]
            if (cmdString[++char_counter] != 'S') {
              return (STATUS_INVALID_STATEMENT);
            }
            if (cmdString[++char_counter] != 'T') {
              return (STATUS_INVALID_STATEMENT);
            }
          ...
          default :  // Storing setting methods [IDLE/ALARM]
            if (!readFloat(cmdString, char_counter, parameter)) {
            ...
            if (!readFloat(cmdString, char_counter, value)) {
              return (STATUS_BAD_NUMBER_FORMAT);
            }
            if ((cmdString[char_counter] != 0) || (parameter > 255)) {
              return (STATUS_INVALID_STATEMENT);
            }
